...
| 0022 | PASS, PR #40 | `infra/supabase/` Terraform adoption scaffold (plan-only, apply removed), `.github/workflows/v2-db-smoke.yml` (disposable Postgres smoke on PRs), `smoke.ts` (verifies 8 core tables + `idx_projects_org` + `lifecycle_status` check constraint), plan-first provisioning workflow. Verifier fixed `smoke.ts` check-constraint query and removed premature apply job. CI 5/5 green. Live Supabase DB and Hyperdrive blocked by missing credentials (expected). Merged as squash commit e784ba1. |
| 0023 | READY | Prompt created at `ai/tasks/task-0023.md`: implement first V2 API route slice for org/project onboarding. Scope: Worker-safe `@orun/db` exports, V2 auth context and role/permission helpers, Postgres-backed `/v2/me`, organization create/list/get, and project create/list/get routes. Must preserve V1 behavior, avoid Node-only DB imports in Worker, and either implement or explicitly propose/defer the policy/audit gap. |
