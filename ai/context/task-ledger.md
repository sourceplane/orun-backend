...
| 0021 | READY | Prompt created for first V2 implementation: create `packages/db`/`@orun/db`, add Postgres migration harness, and check in bounded core schema migration. Must not provision Supabase, add Hyperdrive, add `/v2` routes, or change V1 behavior. |
