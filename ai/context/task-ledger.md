...
| 0020 | READY | Implementer prompt created for `sourceplane/orun`: complete `orun backend init/status/destroy` so self-hosted bootstrap provisions D1/R2/Worker/Queues/DLQ/consumer/cron, embeds migration 0006+, verifies Cloudflare resource creation and endpoint smoke, and opens a PR. |
